...
const { ReopenPing } = require("../../../config.json");
...
module.exports = {
  ...
  async execute(interaction, client, reasone) {
    ...
    const ticketindex = await db.get(
      `ticket.${ticketowner.id}.ticket[${index}]`,
    );

    if (ticketindex.status) {
return await interaction.reply({
        content: "## Ticket already open!",
        ephemeral: true,
      });
}
    ...
    if (ReopenPing) {
      const userembed = new EmbedBuilder()
        ...
        .setTitle("Ticket Reopen!")
        .setDescription(
          `Hey your ticket has been reopened in below\n${interaction.channel}`,
        )
