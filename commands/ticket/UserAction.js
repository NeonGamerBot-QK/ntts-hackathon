...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName("list").setDescription("see all user in the ticket!"),
    )
    .addSubcommand((subcommand) =>
      subcommand
        .setName("add")
        ...
        .addUserOption((option) =>
          option
            .setName("user")
            .setDescription("The user to be add of the ticket")
            .setRequired(true),
        ),
    )
    .addSubcommand((subcommand) =>
  ...
  async execute(interaction) {
    ...
    const index = userticket.findIndex(
      (user) => user.channel === interaction.channel.id,
    );
    ...
    async function list(interaction) {
      ...
      const userarray = await db
        .get(`ticket.${topic}.ticket[${index}].user`)
        .then(async (ar) => {
          return ar;
        });
...
return interaction.reply({
          content: `## You can't add a bot to the ticket!`,
          ephemeral: true,
        });
}

      const include = await db
        .get(`ticket.${ticketowner.id}.ticket[${index}].user`)
        .then((ar) => {
...
return interaction.reply({
      ...
      const userembed = new EmbedBuilder()
        ...
        .setDescription(
          `Hey ${user} You have been added to the ticket \n${interaction.channel}!`,
        )
        .setTimestamp();
      ...
      try {
        ...
        interaction.channel.send({
          content: `## Hey ${user}, Admin add you to this ticket!`,
        });
      }
