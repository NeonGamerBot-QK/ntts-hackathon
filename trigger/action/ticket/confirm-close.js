...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const row2 = new ActionRowBuilder().addComponents(reopenbtn);

    if (AutoDeleteTicket) {
      const time = await create(10, "Relative");
      await interaction.channel.send(
        `##This ticket will be close in ${time}\nIf this is a mistake, please click the button below to reopen the ticket.`,
        { components: [row2] },
      );
      return setTimeout(() => {
    ...
    const embed = new EmbedBuilder()
      .setTitle("Ticket Close")
      .setDescription(
        "This ticket is close, click Button below to delete this ticket",
      )
      .setTimestamp();
